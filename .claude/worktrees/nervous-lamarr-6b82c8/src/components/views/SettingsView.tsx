...
const SKU_OPTIONS = ['essentials', 'pro', 'dispatch', 'dispatch_pro', 'rail', 'exports', 'maintenance_essentials', 'maintenance_pro', 'maintenance_enterprise']
const SKU_LABELS: Record<string, string> = {
  ...
  exports:                'Exports',
  // ReSpark — Maintenance
  maintenance_essentials: 'Maint. Essentials',
  maintenance_pro:        'Maint. Pro',
  maintenance_enterprise: 'Maint. Enterprise',
}
...
function PlanStructureEditor({ template, sessionTemplates, trainingTemplates, onSave }: { template: PlanTemplate; sessionTemplates: SessionTemplate[]; trainingTemplates: TrainingTemplate[]; onSave: (s: PlanTemplate['structure']) => Promise<unknown> }) {
  ...
  const addSubItem = (mi: number, si: number, ii: number, text: string) => {
    const existing = structure.milestones[mi].stages[si].items[ii].checklist || []
    updateItem(mi, si, ii, { checklist: [...existing, { text }] })
  }
  const removeSubItem = (mi: number, si: number, ii: number, ci: number) => {
    const existing = structure.milestones[mi].stages[si].items[ii].checklist || []
    updateItem(mi, si, ii, { checklist: existing.filter((_, k) => k !== ci) })
  }
              ...
              {milestone.stages.map((stage, si) => (
                <div key={si} style={{ borderTop: '1px solid var(--border)' }}>
                  ...
                  {stage.items.map((item, ii) => (
                    <div key={ii}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '4px 12px 4px 44px', flexWrap: 'wrap' }}>
                    ...
                    {item.type === 'task' && (
                      <div style={{ paddingLeft: 60, paddingRight: 12, paddingBottom: 4 }}>
                        {(item.checklist || []).map((sub, ci) => (
                          <div key={ci} style={{ display: 'flex', alignItems: 'center', gap: 4, padding: '1px 0' }}>
                            <span style={{ fontSize: 9, color: 'var(--text-4)', flexShrink: 0 }}>—</span>
                            <span style={{ fontSize: 11, color: 'var(--text-2)', flex: 1 }}>{sub.text}</span>
                            <button onClick={() => removeSubItem(mi, si, ii, ci)}
                              style={{ background: 'none', border: 'none', color: '#ef444466', fontSize: 11, cursor: 'pointer', padding: '0 2px' }}
                              onMouseEnter={e => (e.currentTarget.style.color = '#ef4444')}
                              onMouseLeave={e => (e.currentTarget.style.color = '#ef444466')}>×</button>
                          </div>
                        ))}
                        <input
                          placeholder="Add sub-item…"
                          style={{ width: '100%', background: 'none', border: 'none', borderBottom: '1px solid var(--border)', outline: 'none',
                            color: 'var(--text)', fontSize: 11, padding: '2px 0', fontFamily: 'var(--font-ui)', boxSizing: 'border-box' }}
                          onKeyDown={e => {
                            if (e.key === 'Enter' && e.currentTarget.value.trim()) {
                              addSubItem(mi, si, ii, e.currentTarget.value.trim())
