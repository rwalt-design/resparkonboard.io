...
// ─── Stage classification (for badges + notes + Go-Live marker) ────────────────
...
const CUSTOMER_PREFIXES   = ['return ', 'submit ']
const NOTE_STAGES         = new Set(['user testing', 'uat', 'post launch'])
const GO_LIVE_BEFORE      = new Set(['post launch', 'post launch check-in'])

function ownerOf(item: Item, stageLower: string): 'customer' | 'session' | 'respark' {
  if (item.type === 'session') return 'session'
  if (CUSTOMER_STAGES.has(stageLower)) return 'customer'
  if (item.type === 'task' || item.type === 'exchange') {
    const n = (item.task_name || '').toLowerCase()
    if (CUSTOMER_PREFIXES.some(p => n.startsWith(p))) return 'customer'
  }
  return 'respark'
}
...
const SKU_LABELS: Record<string, string> = {
  essentials: 'Essentials', pro: 'Pro', dispatch: 'Dispatch', rail: 'Rail',
  exports: 'Exports', uptimepm_core: 'UptimePM Core', uptimepm_pro: 'UptimePM Pro',
...
const HARDWARE_TYPE_LABELS: Record<string, string> = {
  floor_scale: 'Floor Scale', truck_scale: 'Truck Scale',
  camera: 'Camera', tablet: 'Tablet', other: 'Other',
}
const CATEGORY_LABELS: Record<string, string> = {
  government_upload: 'Gov Upload', regulatory_config: 'Regulatory',
  document_template: 'Doc Template', other: 'Other',
}

// ─── Styles ────────────────────────────────────────────────────────────────────

const TEAL = '#1BB3BB', DARK = '#1e293b', GRAY = '#64748b', LIGHT = '#94a3b8'
const BGALT = '#f8fafc', BORDER = '#e2e8f0'

const s = StyleSheet.create({
  page: { fontFamily: 'Helvetica', backgroundColor: '#ffffff', paddingTop: 40, paddingBottom: 40, paddingHorizontal: 48 },
  coverPage: { fontFamily: 'Helvetica', backgroundColor: '#ffffff', paddingTop: 48, paddingBottom: 40, paddingHorizontal: 56, display: 'flex', flexDirection: 'column' },
  ...
  noteRow:  { flexDirection: 'row', paddingHorizontal: 10, paddingLeft: 18, paddingVertical: 7, borderTopWidth: 1, borderTopColor: '#f1f5f9', borderTopStyle: 'solid' },
...
function Checkbox({ done }: { done: boolean }) {
  return done ? (
    <View style={s.checkboxChecked}><Text style={s.checkmark}>✓</Text></View>
  ) : (
...
export function ExportPlanPDF({
  account, hardwareTasks, reportTasks, complianceTasks, rep, logoSrc, today,
}: {
  account: Account
  ...
  return (
    <Document>
      ...
      <Page size="LETTER" style={s.page}>
        ...
        {visibleMilestones.map((milestone, mi) => {
          ...
          milestone.stages.forEach(stage => {
            ...
            // Items are already filtered to selected-only by the API route
            ...
            if (GO_LIVE_BEFORE.has(stageLower) && !goLiveInserted) {
              goLiveInserted = true
              stageBlocks.push(
            ...
            stageBlocks.push(
              <View key={stage.id}>
                ...
                {items.map(item => {
                  ...
                  const owner = ownerOf(item, stageLower)
                  return (
                    <View key={item.id} style={s.itemRow}>
                      <Checkbox done={done} />
                      <Text style={done ? s.itemLabelDone : s.itemLabel}>{label}</Text>
                      {owner === 'session'  && <Text style={s.sessionBadge}>session · ReSpark</Text>}
                      {owner === 'customer' && <Text style={s.customerBadge}>customer</Text>}
                      {owner === 'respark'  && <Text style={s.resparkBadge}>ReSpark</Text>}
                    </View>
                  )
                })}
          ...
          <View style={s.tableWrapper}>
            ...
            {hardwareTasks.map((task, idx) => (
              <View key={task.id} style={[s.tableRow, idx === hardwareTasks.length - 1 ? { borderBottomWidth: 0 } : {}]}>
                <View style={{ width: 20, marginRight: 6 }}><Checkbox done={task.completed} /></View>
      ...
      {(reportTasks.length > 0 || complianceTasks.length > 0) && (
        <Page size="LETTER" style={s.page}>
              ...
              <View style={s.tableWrapper}>
                ...
                {reportTasks.map((task, idx) => (
                  <View key={task.id} style={[s.tableRow, idx === reportTasks.length - 1 ? { borderBottomWidth: 0 } : {}]}>
                    <View style={{ width: 20, marginRight: 6 }}><Checkbox done={false} /></View>
